...
def get_gpt4o_mini_answer(query, msg):
    ...
    if any(phrase in answer for phrase in non_satisfactory_answers):
        ...
        data.append({"question": query, "answer": new_answer})  # Cambia a formato dict
        save_training_data()
        retrain_model()  # Llama a la función de reentrenamiento
...
def get_response_with_fallback(question, user_id):
    ...
    try:
        ...
        if local_response and local_response != "Lo siento, no tengo información sobre eso.":
    ...
    try:
        ...
        write_db(db_data)
        return "Lo siento, no tengo una respuesta ahora. Tu consulta ha sido registrada para futuras mejoras."
    except Exception as e:
...
def retrain_model():
    ...
    # Generar un mapeo único para respuestas
    unique_answers = list(set(answers))
    answer_to_index = {answer: idx for idx, answer in enumerate(unique_answers)}
    labels = [answer_to_index[answer] for answer in answers]
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        print(f"Época {epoch+1}/{Config.NUM_EPOCHS}, Pérdida de validación: {val_loss:.4f}")

        if val_loss < best_loss:
            best_loss = val_loss
            patience_counter = 0
            # Guardar el modelo si se encuentra una mejor pérdida
            save_model(model)
        else:
            patience_counter += 1
            if patience_counter >= Config.PATIENCE:
                print("Entrenamiento detenido por paciencia (Early Stopping)")
                break
